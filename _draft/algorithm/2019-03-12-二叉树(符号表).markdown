...
- 为了遍历二叉树，利用Util.stack来保存，然后在用stack来迭代。然后就可以用for来迭代这个数据结构了。
...
```java
 public Iterator iterator() {
    ...
    private class BinTreeIterator<V> implements Iterator{
        ...
        private void buildIterator(Node x,Stack stack){
            if (x==null) return;
            stack.add(x.value);

            buildIterator(x.left,stack);
            buildIterator(x.right,stack);
        }
